...
async def validate_wager(user_id: int, raw_amount: str, game: str = "casino") -> tuple[bool, int | str]:
    ...
    # Event wager-cap boosts (e.g. High Rollers Week) also lift the absolute ceiling.
    ceiling = int(MAX_CASINO_WAGER * wager_cap_multiplier(game))
    cap = min(await max_wager_for(user_id, game), ceiling)
    if amount > cap:
